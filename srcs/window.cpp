...
/*   Updated: 2019/12/04 14:34:58 by ebatchas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Mwindow::process_input(void)
{
	...
	Cube		cube = Cube();
	Cube		water = Cube();
	Camera		camera = Camera(glm::vec3(0.0, 10.8, 100.0), glm::vec3(0.0, 0.0, -1.0));
	...
	terrain.Load(DEFAULT_TERRAIN_SIZE, this->map);
	cube.Load();
	water.Load();
	skybox.Load();
	last_time = SDL_GetTicks();
	sph.InitSPH(terrain.height, terrain.max_height);
	...
	while (not input.quit)
	{
		...
		M = glm::scale(M, glm::vec3(150.0, 150.0, 150.0));
